...
Begin VB.Form yb12 
   ...
   Begin TabDlg.SSTab SSTab1 
      ...
      TabHeight       =   520
      TabCaption(0)   =   "流量过程预报"
      TabPicture(0)   =   "Yb12.frx":09C5
      Tab(0).ControlEnabled=   0   'False
      Tab(0).Control(0)=   "DataGrid1"
      Tab(0).ControlCount=   1
      TabCaption(1)   =   "预报特征值"
      TabPicture(1)   =   "Yb12.frx":09E1
      Tab(1).ControlEnabled=   -1  'True
      Tab(1).Control(0)=   "DataGrid2"
      Tab(1).Control(0).Enabled=   0   'False
      Tab(1).ControlCount=   1
      TabCaption(2)   =   "时段特征值"
      TabPicture(2)   =   "Yb12.frx":09FD
...
Dim it As Long, yy As Integer, dd As Integer, mm As Integer
'Combo1.Text = "断面"
...
' Combo2.Text = "请选择洪水"
 it = glchsdsj(1)
 Call ymd(it, yy, mm, dd)
 Combo2.Text = yy
  rs2 = "select 年份 from" + " ybresud" + CStr(dyly) + " order by 年份"
...
bname = "ybresud" + CStr(dyly)
sql1 = "select *  from " + bname + " order by dt"
...
bname = "ybresud" + CStr(dyly) + "1"
sql1 = "select * from " + bname + " order by 年份"
With Adodc4
  .ConnectionString = kname
  .RecordSource = sql1
End With
...
yy = Int(dt / 10000)
m = Int((dt Mod 10000) / 100)
d = Int(((dt Mod 10000) Mod 100) / 1)
...
dtt = Combo2.Text
rs1 = "select * From  " + "ybresud" + CStr(dyly) + "  where 年份=" & dtt & " order by dt"
...
dtbg = todate(.Fields("dt"))
 ...
 dtt = todate(.Fields("dt"))
  ...
  TChart1.Series(3).Add .Fields("上游来水演算流量"), dtt, clTeeColor
'errhandle:
'    MsgBox "数据库访问出错"
