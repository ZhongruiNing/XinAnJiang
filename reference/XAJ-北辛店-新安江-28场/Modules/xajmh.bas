...
Sub ybgenera()
 ...
 Next CountFlood
 'MsgBox "次模洪水计算完毕！", vbInformation, "提示信息"
  Call cishow
  yb09.Show
  Beep
End Sub
...
Dim nna As Integer, ia As Integer, j As Integer, i As Integer
...
Dim emy As Single, eky As Single, eey As Single, area As Single

Dim ce As Single, qom As Single, qcm As Single, eqm As Single
Dim iom As Integer, icm As Integer, iem As Integer, dc As Single

''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
Dim dc2 As Single, dc3 As Single, dc4 As Single
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''

Dim robsy As Single, rcaly As Single, rin As Single
Dim pj() As Single, qcal() As Single, qwater() As Single, rrcc As Single

m = LongTime

ReDim evap(m + 3 * Int(24 / gltt))
Call eevap(evap)

Dim qin() As Single, qinh() As Single
ReDim qin(LongTime), qinh(LongTime)

Call inputparah(para1, para2, para3, Na, ia)
ReDim zdylp(glnn, Na), state(Na, 7)

Call inputrain(Na, zdylp, para3)
Call stateh(Na, state)

ReDim qobs(m + 10), qjy(m)

Call inputq(qobs)

Call chmybH(para1, para2, evap, pj, qcal, qobs, zdylp, state, rr, Na, ia, wj, emy, eky, eey, area, rrcc)
Call qinflowh(ia, qc, qrc)
For i = 1 To m
      qin(i) = qc(i)
      qinh(i) = qrc(i)
Next i
...
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
Call Charact_Watershed_Q(qcal, qobs, qin, m, rin, robsy, rcaly, ce, qom, qcm, eqm, iom, icm, iem, dc, _
                         area, dc2, dc3, dc4)
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
...
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
Call savebbo(wo, wj, pj, qin, qinh, qjy, qcal, qobs, rr, rin, robsy, rcaly, ce, qom, qcm, eqm, iom, icm, _
             iem, dc, m, emy, eky, eey, area, rrcc, ew, dc2, dc3, dc4)
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''

End Sub
Sub chmybH(para1() As Single, para2() As Single, evap() As Single, _
       ...
       emy As Single, eky As Single, eey As Single, area As Single, rrcc As Single)
...
'rst.Open bname, cnn, adOpenDynamic, adLockOptimistic
'cnn.Execute (sql1)
...
'  For i = 1 To lp - 1
'        Write #3, wup(i), wlp(i), wdp(i), frp(i), sp(i);
'  Next i
'  Write #3, wup(lp), wlp(lp), wdp(lp), frp(lp), sp(lp)
...
'      Write #3, CountFlood, j, i, r
  ...
  '   rst(1) = it
  '   rst(2) = i
  '   rst(3) = Int(100 * ee) / 100
  '   rst(4) = Int(100 * pp(i)) / 100
  '   rst(5) = Int(100 * wp(i)) / 100
  '   rst(6) = Int(100 * wup(i)) / 100
  '   rst(7) = Int(100 * wlp(i)) / 100
  '   rst(8) = Int(100 * wdp(i)) / 100
  '   rst(9) = Int(100 * qcal(j - 1) / lp) / 100
  '   rst(10) = Int(100 * sp(i)) / 100
  '   rst(11) = Int(100 * frp(i)) / 100
  '   rst.Update
   ...
   Next i
      ...
      qcal(j) = qsig(j)
'      Write #3, CountFlood, j, rsy, riy, rgy, qsz, qiz, qgz
  ...
  '  rst(3) = Int(100 * ee) / 100
  '  rst(4) = Int(p * 100) / 100
  '  rst(5) = Int(wp(1) * 100) / 100
  '  rst(6) = Int(wwu * 100) / 100
  '  rst(7) = Int(wwl * 100) / 100
  '  rst(8) = Int(wwd * 100) / 100
  '  rst(9) = Int(100 * qcal(j)) / 100
  '  rst(10) = Int(100 * ss) / 100
  '  rst(11) = Int(ffr * 100) / 100
  '  rst.Update
Next j
'cnn.Close
Close #3
